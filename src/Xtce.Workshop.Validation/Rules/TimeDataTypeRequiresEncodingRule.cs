...
/// set"). The Encoding wrapper is modeled since #100 (fragment presence still counts for
/// hand-built documents). A type carrying a baseType attribute (in PreservedAttributes)
/// may inherit its encoding from the base; resolving time-type inheritance chains is out
/// of scope, so baseType presence skips the check rather than risking a false positive.
/// </summary>
public sealed class TimeDataTypeRequiresEncodingRule : IValidationRule
{
    ...
    public IEnumerable<ValidationIssue> Validate(SpaceSystemContext context)
    {
        foreach (var type in context.Node.TelemetryMetaData?.ParameterTypeSet ?? [])
        {
            ...
            var hasEncoding = type.TimeEncoding is not null
                || (type.Preserved ?? []).Any(f => f.ElementName == "Encoding");
